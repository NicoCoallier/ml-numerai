...
from staker.constants import VAL_SIZE, SHUFFLE_FACTOR
...
class NumerAIDataset:
    ...
    def download(self)->NoReturn:
        ...
        """
        self.napi.download_current_dataset(unzip=True, dest_path="./data/", dest_filename="current_round")
    ...
    def whiten(self, data:pd.DataFrame, num_components:int = 80)->pd.DataFrame:
        ...
        print('Explained variation for all components: {}'.format(np.sum(pca.explained_variance_ratio_)))
        return data[self.to_keep]
    ...
    def preprare_data(self, data:tf.data.Dataset, training:bool=True)->tf.data.Dataset:
